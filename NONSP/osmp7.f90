...
! +++++Variables locales
      ...
      REAL(kind=8), DIMENSION(nvar) ::  delu
...
! +++++Flux EULER
      ...
      call flux_euler(dir, -nordre+1, nordre, u_n, fi, gh, temperature )
...
! +++++moyenne de Roe
      ...
      do l = -nordre+1,nordre-1
        ...
        ecp12(l) = 0.
        do m = 2, nvar-1
          up12(m-1,l) = (cof*u_n(m,l+1)/u_n(1,l+1) + &
                             u_n(m,l)/u_n(1,l))*usupcof
...
! +++++valeurs propes aux demi-mailles
      ...
      do l = -nordre+1,nordre-1
        do m = 1, nvar-2
          vp(l,m) = up12(dir,l)
        enddo
        vp(l,nvar-1) = up12(dir,l) + cp12(l)
        vp(l,nvar) =   up12(dir,l) - cp12(l)
      enddo
...
!
      ...
      do m = 1, nvar
        ...
        C_mp(m) = C_mp(m)*cp12(0) + 0.5 * ( fi(m, 1) + fi(m, 0) )
