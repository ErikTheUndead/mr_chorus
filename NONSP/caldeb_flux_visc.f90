      subroutine caldeb_flux_visc(maille, dir, fvisc_gauche)
      ...
      REAL(kind=8), DIMENSION(nvar) :: fvisc_gauche
      ...
      REAL(kind=8) :: mu_wall
      REAL(kind=8) :: divV, divrhoV, T_xyz
      REAL(kind=8), DIMENSION(ndim, ndim) :: u_xyz, rhoV_xyz
...
! *****Projection des flux Euler dans l'espace physique
        ...
        do m = 1, nvar
          fvisc_gauche(m) = maille%fvisc_dt(m, dir)
        enddo
...
! .....vicosite dynamique a la paroi ==> extrapolation (ordre 3)
...
! .....Composantes du Flux visqueux
        fvisc_gauche(1) = 0.
        do m = 2, nvar-1
          fvisc_gauche(m) = - mu_wall*( u_xyz(dir,m-1) + u_xyz(m-1,dir) - &
...
! .....gradient de Temperature suivant "dir"
          ...
          fvisc_gauche(nvar) = - mu_wall*T_xyz / &
      ...
      end subroutine caldeb_flux_visc
