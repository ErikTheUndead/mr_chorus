...
! +++++Variables locales
      ...
      REAL(kind=8), DIMENSION(-nordre+1:nordre-1) :: cp12, hp12, ecp12,c12
      ...
      REAL(kind=8), DIMENSION(-nordre+1:nordre,nvar) :: vp, vp2, xnu
...
!       call flux_euler_acc(dir, -nordre+1, nordre, u_n, gh, temperature,c12)
...
! +++++moyenne de Roe
      ...
      do l = -nordre+1,nordre-1
        ...
        cof = sqrt(u_n(1,l+1)/u_n(1,l))
        usupcof=1./(1.+cof)

        ecp12(l) = 0.
        do m = 2, nvar-1
          up12(m-1,l) = (cof*u_n(m,l+1)/u_n(1,l+1) + u_n(m,l)/u_n(1,l))*usupcof
          ecp12(l) = ecp12(l) + up12(m-1,l)**2
        enddo
        ecp12(l) = 0.5 * ecp12(l)

        hp12(l) = (cof*gh(l+1)+gh(l))*usupcof
