      subroutine calfin_flux_euler(maille, dir, fmp_droit)
...
! *****Projection des flux Euler sur les vecteurs propres
        ...
        do m = 1, nvar
          ...
          if( vp(m) > 0. ) then
            do l = 1, nvar
              dfmp(m) = dfmp(m)  + &
                 Lip12(1,m,l)*(fmp_droit(l)-maille%fmp_gch(l))
            enddo
          endif
...
!!          rhoV_xyz(m-1, dir) = (-9.*u_n(m, 1) + u_n(m, 2)) &
!!                                 / (3.*maille%dx(dir))
        enddo            
...
!      pression paroi extrapolee (ordre 3)
        ...
        fmp_droit(dir+1) = p_local
!!
!!!      Pression paroi ==> d/dn pression = 0 - ordre 3
!!        fmp_droit(dir+1) = ( 225.*psgM2(1) - 50.*psgM2(2) + 9.*psgm2(3)) &
!!                           / 184.
!!!      pression paroi = rho * T
!!        fmp_droit(dir+1) = rho_wall * t_local

      endif

      end subroutine calfin_flux_euler
