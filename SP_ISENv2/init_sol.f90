...
! +++++initialisation convection tourbillon isentropique
       ...
       u_theta = 0.5*ampli*exp(.5*(1.-rayon**2)) / pi
       delta_temp = -(gamma-1.)*ampli**2*exp(-rayon**2) / &
                     (8.*pi**2)
...
! +++++initialisation d'un spot de temperature
       ...
       delta_temp = ampli*exp(-c2*rayon**2)
...
! +++++Plane Taylor Vortex
         ...
         rho_ec = 0.5 * rho_ec / maille%u(1)
			...
			maille%u(nvar) = p_left/(gamma*(gamma-1.)*Mach**2) + 0.25*(cos(2*pi*maille%x(1)) + cos(2*pi*maille%x(2)))/(gamma-1.) + rho_ec
