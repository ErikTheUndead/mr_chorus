      subroutine flux_osmp7_acoustic(dir)
      ...
      REAL(kind=8), DIMENSION(2) :: C_mp, flux_acc
...
! +++++Calcul de la correction OSMP7 a la frontiere amont du domaine
          ...
          call osmp7_acoustic(dir, sigma, u_n, C_mp, flux_acc)

! .....Correction TVD-MP a gauche de la maille courante

					do l = 1,2
						maille%fmp_gch(l) = C_mp(l) ! Cj+1/2
						maille%f_gch(l) = flux_acc(l) ! Fj+1/2
					end do
...
! .....Somme des flux pour conservation
				...
				do l = 1,2
					fmp_droit(l) = fmp_droit(l) + &
													C_mp(l) / float(2**(ndim-1))

					f_droit(l) = f_droit(l) + &
													flux_acc(l) / float(2**(ndim-1))
				end do
...
! .....indice de la maille suivante
            ijk_gch = ijk_dt( l_dt,dir ) - 2**(dir-1)
				...
				do l =1,2
					maille%support(dir)%suivant%fils(ijk_gch)%ptr%fmp_gch(l) = C_mp(l)
					maille%support(dir)%suivant%fils(ijk_gch)%ptr%f_gch(l) = flux_acc(l)
				end do
...
! .....Correction TVD-MP a droite pour la maille courante
					...
					do l = 1,2
						maille%fmp_dt(l) = fmp_droit(l)
						maille%f_dt(l) = f_droit(l)
					end do
...
! .....Correction TVD-MP a droite pour la maille courante
					...
					do l = 1,2
						maille%fmp_dt(l) = C_mp(l)
						maille%f_dt(l) = flux_acc(l)
					end do
...
! +++++Transfert des flux pour le flux a gauche de la feuille suivante
          ...
          if( associated(maille%support(dir)%suivant) ) then
						...
						do l = 1,2
							maille%support(dir)%suivant%fmp_gch(l) = C_mp(l)
							maille%support(dir)%suivant%f_gch(l) = flux_acc(l)
						end do
      ...
      end subroutine flux_osmp7_acoustic
