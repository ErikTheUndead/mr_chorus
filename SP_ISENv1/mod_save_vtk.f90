MODULE mod_save_vtk
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	write(filename,*) echantillonname,'_1DSP_ISENv1',iterationname,'.dat'
...
! 	999   format(A3,'_1DSP_ISEN','_niv',i2.2,'_rhoL1s',i1,'_eps',1pe6.0,'/')
...
!   call system('DIR_WORK=~/duhem/stage_2014_Limsi/WORK_DIR')
  write(filename,*) echantillonname,'_2DSP_ISENv1_',iterationname,'.vtk'
  write(foldername,*) trim(adjustl(nom_fich)),'.folder'
  full_name = 'WORK_DIR/' // trim(adjustl(foldername)) // '/' // trim(adjustl(filename))
  full_name = trim(full_name)
  foldername = trim(adjustl(foldername))
