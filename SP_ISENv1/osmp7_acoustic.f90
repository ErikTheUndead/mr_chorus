...
! +++++Variables arguments
      ...
      REAL(kind=8), INTENT(OUT), DIMENSION(2) ::  C_mp,flux
...
! +++++Variables locales
      ...
      REAL(kind=8), DIMENSION(-nordre+1:nordre-1) :: cp12, hp12, ecp12, rho12
      REAL(kind=8), DIMENSION(3, -nordre+1:nordre-1) :: up12
      REAL(kind=8), DIMENSION(-nordre+1:nordre-1,2,2) :: Rip12, Lip12

      REAL(kind=8), DIMENSION(-1:1) :: d4i, d4ip1, d4min, dmm, dm4
      REAL(kind=8), DIMENSION(-1:1) :: dip12
      REAL(kind=8), DIMENSION(-nordre+1:nordre) :: gh
      ...
      REAL(kind=8), DIMENSION(2) ::  phi_lim
      REAL(kind=8), DIMENSION(-nordre+1:nordre) :: co7_1, co7_2, &
                                                   co7_3, co7_4, co7_5

      REAL(kind=8), DIMENSION(2) ::  delu, deluf
      REAL(kind=8), DIMENSION(-nordre+1:nordre,2) ::  delw, delwnu
      REAL(kind=8), DIMENSION(-nordre+1:nordre,2) :: vp, xnu

! +++++Calcul des variables thermodynamiques pour l'acoustique

       call flux_euler(-nordre+1, nordre, u_n, gh)
...
! +++++valeurs propres aux demi-mailles + CFL local
      ...
      do l = -nordre+1,nordre-1
        vp(l,1) =  + cp12(l)
        vp(l,2) =  - cp12(l)
        xnu(l,1) = sigma * abs(vp(l,1))
        xnu(l,2) = sigma * abs(vp(l,2))
      enddo

! +++++vecteurs propres : matrice a gauche et à droite

      call vcalir_acc(-nordre+1, nordre-1,rho12, cp12, Lip12, Rip12)

! +++++Invariants de Riemann

      do l = -nordre+1,nordre-1
          delu(1) = u_n(1,l+1) - u_n(1,l)
			 delu(2) = u_n(dir+1,l+1)/u_n(1,l+1) - u_n(dir+1,l)/u_n(1,l)
